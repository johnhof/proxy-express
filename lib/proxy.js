...
function buildStream (host, options, secure) {
  ...
  var preSet = options && options.digest && options.digest.pre;
  if (preSet) {
  ...
  var postSet = options && options.digest && options.digest.post;
  if (postSet) {
    // accept raw functions, but wrap it in an array
...
function buildResultingMiddleware (host, options, stream) {
  return function proxyMiddleware (req, res, next) {
    ...
    var _stream = _.clone(stream, true);
    ...
    var reqOpts = {
      ...
      method  : req.method,
      // headers : _.defaults(options.headers || {}, req.headers),
      headers : options.headers,
      form    : req.body,
    };

    // if the route is greenlit, add the proxy option builder to the stream
    _stream.unshift(function (callback) {
...
console.log('FINAL STREAM: ', _stream)
    return async.waterfall(_stream, respond(next));
  }
}
...
console.log('firing proxyReq', proxyObj.reqOpts)
  ...
  request(proxyObj.reqOpts, function (error, response, body) {
    ...
    // transfer result to resulting object
    proxyObj.proxyDone = true;
    proxyObj.result = {
      response : response,
      body     : body
    };
...
function respond (next) {
  return function sendResult (error, proxyObj) {
    ...
    if (proxyError) {
      ...
      return next(new Error('Proxy to ' +  + ' failed for an unkown reason'));

    } else {

      proxyObj.res.set(proxyObj.result.response.headers)
